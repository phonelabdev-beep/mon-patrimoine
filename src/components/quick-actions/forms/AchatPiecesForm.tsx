...
export default function AchatPiecesForm() {
  ...
  function handleSubmit(e: FormEvent) {
    ...
    try {
      achatPieces({ date, compteId, libelle: libelle.trim() || 'Achat de pièces', lignes: lignesInput })
      navigate('/pieces')
    } catch (err) {
            ...
            <div key={i} className="flex flex-col gap-2 rounded-lg border border-zinc-700 p-3">
              ...
              {l.mode === 'existante' ? (
                ...
                <div className="flex flex-col gap-2">
                  <input className={inputClass} value={l.nom} onChange={(e) => majLigne(i, { nom: e.target.value })} placeholder="Nom (ex : Écran iPhone 11 OLED)" />
                  <input className={inputClass} value={l.categorie} onChange={(e) => majLigne(i, { categorie: e.target.value })} placeholder="Catégorie (écran, batterie…)" />
